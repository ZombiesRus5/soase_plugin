...
public class VanillaWikiDefinitionBuilder implements DefinitionHandler {
	...
	public static void main(String[] args) throws Exception {
		...
		parser.setStrictValidation("Vanilla");
	...
	public VanillaWikiDefinitionBuilder(String definitionName) throws IOException {
		pw.push(new PrintWriter(new FileWriter(new File("C:/Users/LaffoonGame/workspace/Sins Wiki/Vanilla" + definitionName + ".wiki"))));
	}
	...
	public void field(String fieldName, String fieldRule, boolean explicit, String[] fieldValues, String fieldType, String[] references, String helpText) throws Exception {
		...
		if ((fieldValues != null && fieldValues.length >0 && "Enumeration".equals(fieldType)) || (helpText != null && helpText.trim().length() > 0)) {
			print("[Vanilla" + fieldRule + " " + fieldName + "]: [" + fieldType + "]");
			println();
			...
			if (explicit) {
				...
				PrintWriter pwField = new PrintWriter(new FileWriter(new File("C:/Users/LaffoonGame/workspace/Sins Wiki/Vanilla" + fieldRule + ".wiki")));
				...
				if (fieldValues != null && fieldValues.length > 0) {
					...
					for (int i=0; i<references.length; i++) {
						pwField.print(" * [");
						pwField.print("Vanilla" + references[i] + " " + references[i]);
						pwField.println("]");
					}
					pwField.println();
					...
					for (int i=0; i<fieldValues.length; i++) {
						pwField.print(" * ");
						pwField.println("[Vanilla" + fieldValues[i] + " " + fieldValues[i] + "]");
					}
				}
				pwField.flush();
			}
	...
	public void startStructure(String structureName, String structureType, boolean explicit, String[] references, String helpText) {
		if (printStructureReference) {
			if (references != null && references.length > 0) {
				...
				for (int i=0; i<references.length; i++) {
					print(" * [");
					print("Vanilla" + references[i] + " " + references[i]);
					println("]");
				}
				println();
			}
		...
		if (structureName.equals(structureType)) {
			print("[Vanilla" + structureName + " " + structureName + "]");
		} else {
			print("[Vanilla" + structureType + " " + structureName + "]: [" + structureType + "]");
		}
		println();
		depth.push(depth.peek()+1);
		if (!explicit) {
	...
	public void startConditionBlock(String field, String fieldRule, String[] conditions,
		...
		print("[Vanilla" + fieldRule + " " + field + "]" + ": [Condition]");
		println();
		depth.push(1);
		
		pw.push(new PrintWriter(new FileWriter(new File("C:/Users/LaffoonGame/workspace/Sins Wiki/Vanilla" + fieldRule + ".wiki"))));
		println("#labels SOASE, DynamicContent");
		...
		print("[Vanilla" + field + " " + fieldRule + "]" + ": [Condition]");
		println();
	}
