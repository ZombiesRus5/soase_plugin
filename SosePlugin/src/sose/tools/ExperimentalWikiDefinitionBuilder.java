...
public class ExperimentalWikiDefinitionBuilder implements DefinitionHandler {
	...
	public ExperimentalWikiDefinitionBuilder(String definitionName) throws IOException {
		pw.push(new PrintWriter(new FileWriter(new File("E:/workspace/zombiesrus5/SosePlugin/wiki/" + definitionName + ".wiki"))));
	}
	...
	public void printHeader(String content, int depth) {
		for (int i=0; i<=depth; i++) {
			pw.peek().print('=');
		}
	...
	public void field(String fieldName, String fieldRule, boolean explicit, String[] fieldValues, String fieldType, String[] references, String helpText) throws Exception {
		...
		if ((fieldValues != null && fieldValues.length >0 && "Enumeration".equals(fieldType)) || (helpText != null && helpText.trim().length() > 0)) {
			print("[" + fieldRule + " " + fieldName + "]: [" + fieldType + "]");
			println();
			...
			if (explicit) {
				...
				PrintWriter pwField = new PrintWriter(new FileWriter(new File("E:/workspace/zombiesrus5/SosePlugin/wiki/" + fieldRule + ".wiki")));
				...
				if (fieldValues != null && fieldValues.length > 0) {
					...
					for (int i=0; i<fieldValues.length; i++) {
						pwField.print(" * ");
						pwField.println("[Constants#" + fieldValues[i] + " " + fieldValues[i] + "]");
					}
				}
				pwField.flush();
			}
...
//		printBullet();
		if (structureName.equals(structureType)) {
			printHeader("[" + structureName + "]", depth.peek());
		} else {
			printHeader("[" + structureName + "]: [" + structureType + "]", depth.peek());
		}
		println();
		depth.push(depth.peek()+1);
		if (!explicit) {
	...
	public void startConditionBlock(String field, String fieldRule, String[] conditions,
		...
		pw.push(new PrintWriter(new FileWriter(new File("E:/workspace/zombiesrus5/SosePlugin/wiki/" + field + ".wiki"))));
		println("#labels SOASE, DynamicContent");
