...
public class WikiDefinitionBuilder implements DefinitionHandler {
	...
	public WikiDefinitionBuilder(String definitionName) throws IOException {
		pw.push(new PrintWriter(new FileWriter(new File("C:/Users/LaffoonGame/workspace/Sins Wiki/" + definitionName + ".wiki"))));
	}
	...
	public void field(String fieldName, String fieldRule, boolean explicit, String[] fieldValues, String fieldType, String[] references, String helpText) throws Exception {
		...
		if ((fieldValues != null && fieldValues.length >0 && "Enumeration".equals(fieldType)) || (helpText != null && helpText.trim().length() > 0)) {
			...
			if (explicit) {
				...
				PrintWriter pwField = new PrintWriter(new FileWriter(new File("C:/Users/LaffoonGame/workspace/Sins Wiki/" + fieldRule + ".wiki")));
	...
	public void startStructure(String structureName, String structureType, boolean explicit, String[] references, String helpText) {
		...
		if (structureName.equals(structureType)) {
			...
			print("[" + structureType + " " + structureName + "]: [" + structureType + "]");
		}
		println();
		depth.push(depth.peek()+1);
		if (!explicit) {
	...
	public void startConditionBlock(String field, String fieldRule, String[] conditions,
		...
		print("[" + fieldRule + " " + field + "]" + ": [Condition]");
		println();
		depth.push(1);
		
		pw.push(new PrintWriter(new FileWriter(new File("C:/Users/LaffoonGame/workspace/Sins Wiki/" + fieldRule + ".wiki"))));
		println("#labels SOASE, DynamicContent");
